...
(gtime -f "USERTIME %U\nSYSTEMTIME %S\nELAPSEDTIME %e\nMEMORYTIME %K" x264 --no-asm --ref 5 --no-fast-pskip --no-mbtree --no-deblock --no-8x8dct --rc-lookahead 40  --no-weightb --no-mixed-refs  -o sintel$numb.flv $trailerlocation) 2> $logfilename
# size of the video
