...
(gtime -f "USERTIME %U\nSYSTEMTIME %S\nELAPSEDTIME %e\nMEMORYTIME %K" x264 --no-asm --ref 5 --no-mbtree --no-8x8dct --no-deblock --rc-lookahead 40  --no-cabac --no-weightb --no-mixed-refs  -o sintel$numb.flv $trailerlocation) 2> $logfilename
# size of the video
